...
Projects don't have to involve coding. A project can be a purely written
investigation — documenting lessons learned, comparing tools conceptually,
answering research questions through reading and analysis, or capturing
findings from experiments you ran outside this repo. The format is the same:
notes.md as a working log, README.md as the final report.

For browser testing and visual verification, use `uvx rodney` (Chrome DevTools
automation) and `uvx showboat` (demo document builder with screenshots).
